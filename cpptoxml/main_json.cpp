...
#define ATTR_LIST(n, v) attrList list_(n, v, indent)
...
private:
    ...
    int indent;
public:
    JSONVisitor(CodeModelItem c, bool r = true):
        resolve_types(r), current_scope(), outer_scope(c), indent(0)
    {
        current_scope << c;
    }
    QString TypeTag(CodeModelItem);
...
TypeInfo JSONVisitor::simplifyType(TypeInfo const &__type, CodeModelItem __scope)
{
...
TypeInfo JSONVisitor::solve(const TypeInfo& t, QStringList scope)
{
    (void)scope;
...
QString JSONVisitor::visit(const TypeInfo& t, QStringList scope)
{
...
QString JSONVisitor::TypeTag(CodeModelItem i)
{
    switch (i->kind())
    {
        TAG_CASE(Scope);
...
public:
    QString &s;
    int &indent;
    explicit attrList(QString n, QString& s_, int& indent_)
        : s(s_)
        , indent(indent_)
    {
        s += INDENT + n.prepend("\"").append("\" : [\n");
        ++indent;
    }
...
QString JSONVisitor::visit(CodeModelItem i)
{
    QString ret;

    ++indent;
    ret += ATTR_STR("type", TypeTag(i));

    current_id = TypeTag(i) + " => " + i->qualifiedName().join("::"); // FIXME: this is debug code
    ...
    {
        if (!n->namespaces().isEmpty())
        {
            ATTR_LIST("namespaces", children);
    ...
    if (i->kind() & _CodeModelItem::Kind_Scope)
    {
        ...
        {
            ATTR_LIST("functions", children);
            foreach(FunctionModelItem n, s->functions())
                children += visit(model_static_cast<CodeModelItem>(n)) + ",\n";
        }

        if (!s->typeAliases().isEmpty())
        {
            ATTR_LIST("typeAliases", children);
    ...
    if (EnumModelItem e = model_dynamic_cast<EnumModelItem>(i))
    {
        ...
        {
            ATTR_LIST("arguments", children);
            foreach(EnumeratorModelItem n, e->enumerators())
            {
                if (n->value() == QString())
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            sourceNames << QString::fromLatin1(argv[i]);
        }
    }
    ...
    foreach(QString sourceName, sourceNames) {
        ...
        sourceInclist << (sourceInfo.absolutePath());
        ...
        if (!noCode)
        {
            JSONVisitor visitor((CodeModelItem)f_model, !dontResolve);
